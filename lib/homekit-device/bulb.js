...
class HomeKitDeviceBulb extends HomeKitDevice {
  constructor(platform, wizDevice) {
    ...
    if (wizDevice.supportsBrightness) {
      this.addBrightnessCharacteristics();
    }

    if (wizDevice.supportsColorTemperature) {
  ...
  addBasicCharacteristics() {
    this.addCharacteristic(Characteristic.On, {
      getValue: async () => {
        return this.wizDevice.lighting.getLightState().then(ls => {
          return !!ls.on_off;
        });
      },
      setValue: async value => {
        return this.wizDevice.lighting.setLightState(
          { on_off: value },
          { transport: 'udp' }
        );
      },
  ...
  addEnergyCharacteristics() {
    this.addCharacteristic(CustomCharacteristic.Watts, {
      getValue: async () => {
        return this.wizDevice.emeter.getRealtime().then(emeterRealtime => {
          return emeterRealtime.power;
        });
      },
    });

    this.wizDevice.on('emeter-realtime-update', emeterRealtime => {
      this.fireCharacteristicUpdateCallback(
